...
def calculate_cogging_torque():
    ...
    with multiprocessing.Pool(processes=4) as pool:
        ...
        csv_write(ModelDir.DATA / 'cogging_toruqe.csv', ['rotorangle', 'Torque'], theta, T)

def plot_cogging_torque():
    theta_sim, T_sim = csv_read(ModelDir.DATA / 'cogging_toruqe.csv')
    theta_ref, T_ref = csv_read(ModelDir.DATA / 'cogging_ref.csv')
