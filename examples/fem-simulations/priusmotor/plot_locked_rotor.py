...
predict = np.poly1d(z)
y = predict(x)
plt.plot(x*4, y, c="r", label="whole model by [9]")
z = np.polyfit(x, y2, a)
predict = np.poly1d(z)
y = predict(x)
plt.plot(x*4, y, c="g", linestyle='--', label="pole model from [9]")
z = np.polyfit(x, y3, a)
predict = np.poly1d(z)
y = predict(x)
plt.plot(x*4, y, c="b", linestyle='dotted', label="pole model by DTD")
