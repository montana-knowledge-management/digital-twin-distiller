...
from digital_twin_distiller.objects import CircleArc
from digital_twin_distiller.objects import Line
...
class PriusMotor(BaseModel):
    ...
    def _add_slice(self, r_outer, r_inner):
        origin = Node(0.0, 0.0)
        u_left = Node(math.cos(math.radians(90 + 45 / 2)), math.sin(math.radians(90 + 45 / 2)))
        u_right = Node(math.cos(math.radians(90 - 45 / 2)), math.sin(math.radians(90 - 45 / 2)))
    ...
    def _add_magnets(self):
        ...
        a = math.tan((90 - self.R3 / 2) * math.pi / 180) * (self.R4 - 0.5)
    ...
    def make_full(self):
        ...
        # Calculating u_l, u_r
        bc_names = list(self.snapshot.boundaries.keys())
        lines = set()
        unitvectors = []
        for key in bc_names:
            if isinstance(self.snapshot.boundaries[key], AntiPeriodicBoundaryCondition):
                bc = self.snapshot.boundaries.pop(key)
                lines = lines.union(bc.assigned)

        for id_i in lines:
            line = self.snapshot.lines[id_i]
            u_i = line.start_pt.unit_to(line.end_pt)
            if u_i not in unitvectors:
                unitvectors.append(u_i)

        u_l, u_r = sorted(unitvectors, key=lambda n: n.x)
        alpha_unit = u_l.angle_to(u_r)
        # residual = math.fmod(2*math.pi, alpha_unit)
        # assert residual < 0.008, f'{residual} > 0.008'
        ...
        while u_r != u_l:
            ...
            for i, ni in enumerate(ng.nodes):
                ng.nodes[i] = mirror_point(origin, u_r, ni)

            for i, li in enumerate(ng.lines):
                ng.lines[i].start_pt = mirror_point(origin, u_r, li.start_pt)
                ng.lines[i].end_pt = mirror_point(origin, u_r, li.end_pt)

            # for i, ci in enumerate(ng.circle_arcs):
            #     ng.circle_arcs[i].start_pt=mirror_point(origin, u_r, ci.start_pt)
            #     ng.circle_arcs[i].center_pt=mirror_point(origin, u_r, ci.center_pt)
            #     ng.circle_arcs[i].end_pt=mirror_point(origin, u_r, ci.end_pt)
            #     ng.circle_arcs[i].apex_pt=mirror_point(origin, u_r, ci.apex_pt)

            print(u_r)
            u_r = u_r.rotate(-alpha_unit)
            self.snapshot.add_geometry(ng)
            g = ng
