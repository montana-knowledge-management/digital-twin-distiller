...
# HTTP client
example_project = MachineLearningProject(app_name="test_name")
...
# HTTP client
example_project = SimulationProject()
server_sim = Server(example_project)
client_sim = TestClient(server_sim.app)
...
class TestIntegratedServer(unittest.TestCase):
    ...
    def test_missing_text_key_ml(self):
        ...
        response = client.post("/process_ml", json=wrong_json, headers={"Content-Type": "application/json"})
    ...
    #     wrong_json = {"test": "Without Text key."}
    #     response = client_sim.post("/process_sim", json=wrong_json, headers={"Content-Type": "application/json"})
    #     print(response.json())
    #     self.assertIsNot(response.status_code, 200)
    #     self.assertDictEqual(
    #         response.json(),
    ...
    def test_set_host(self):
        ...
        serv = Server(example_project)
        serv.set_host(example_host)
        self.assertEqual(serv.host, example_host)

    def test_set_port(self):
        example_port = 123
        serv = Server(example_project)
        serv.set_port(example_port)
        self.assertEqual(serv.port, example_port)